...
def run_single (n, k, G, T, arr):
	...
	H_B = np.identity(2**n) - np.outer(psi, psi)
	H_P = np.diag(np.array([(sum([(expr_g(G[i][j]))*expr_z(Bit(z, i))*expr_z(Bit(z, j)) for i in range(n) for j in range(i)])) for z in range(2**n)], dtype=complex))
